...
- [`pairing.md`](pairing.md) — LAN-primary pairing ceremony. QR + nonce, on-device keypair generation, CSR submitted to the home over pinned-cert HTTPS, home-CA signing, fingerprint recorded in `authorized_clients.json`, device token acquired from the relay's control plane. Also documents the browser registration variant.
...
- [`blob-uplink.md`](blob-uplink.md) — browser blob-uplink wire contract. The full cert-less browser client contract: DATA payload shape (`SBO1/SBR1/SBA1`), HPKE suite + RFC 9180 fixture, the msg1–msg5 pairing framing (`SBP1`; bare 5-byte msg1, u32-BE-prefixed msg2–4), `sig` as raw IEEE-P1363 R‖S (not DER), inner-payload encoding (compact JSON + unpadded base64url), ≤64 KiB chunking, and — because a browser can't set request headers — the **per-route subprotocol carrier matrix** (pair-dial echoes `spl-v1`; the DATA dial offers **no** subprotocol; an un-echoed offer silently fails the browser WS). Read this before touching the browser dial.
