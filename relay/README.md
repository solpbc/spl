...
# Dedicated portal-to-relay bearer credential for canonical owner-purge v1.
# This retains the existing credential pending cross-service confirmation.
# Enter it at the prompt; never put it in shell history:
wrangler secret put PURGE_SECRET --env production

# Retained owner-purge v1 HMAC keys. Enter each at the prompt; never put either
# value in shell history. Both versions are required while v1 accepts them.
wrangler secret put OWNER_PURGE_HMAC_KEY_V1 --env production
wrangler secret put OWNER_PURGE_HMAC_KEY_V2 --env production
```
