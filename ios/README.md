...
- `Sources/Services/AppConfig.swift` — gains spl fields (spl-relay endpoint, device token, paired-home instance id)
- App lifecycle (foreground-only with 20s grace)

## what is new

- `Sources/SPLTunnel/CFTunnelTransport.swift` — replaces `SSHTransport.swift`. Opens the WS to `spl-relay`, runs the spl framing layer, demultiplexes streams back to `GlueHandler`'s API.
